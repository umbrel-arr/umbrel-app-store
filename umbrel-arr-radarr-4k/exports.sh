...
export UMBREL_ARR_RADARR_4K_API_KEY="$(cat "$key_file")"
export UMBREL_ARR_RADARR_4K_URL="http://umbrel-arr-radarr-4k_server_1:7878"
config_file="${EXPORTS_APP_DATA_DIR}/config/config.xml"
if [ ! -s "$config_file" ]; then
  mkdir -p "$(dirname "$config_file")"
  printf '<Config>\n  <AuthenticationMethod>External</AuthenticationMethod>\n  <AuthenticationRequired>DisabledForLocalAddresses</AuthenticationRequired>\n  <ApiKey>%s</ApiKey>\n</Config>\n' "$UMBREL_ARR_RADARR_4K_API_KEY" > "$config_file"
fi
