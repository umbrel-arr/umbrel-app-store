...
class Reconciler:
    def __init__(self, settings=None, client=None):
        ...
        self.arrs = [
            ArrInstance("sonarr", "Sonarr", "Sonarr", self.settings.url("sonarr"), self.settings.key("sonarr"), "v3", "/media/series", "tv", 30985),
            ArrInstance("sonarr-4k", "Sonarr 4K", "Sonarr", self.settings.url("sonarr-4k"), self.settings.key("sonarr-4k"), "v3", "/media/series-4k", "tv-4k", 30986, True),
            ArrInstance("radarr", "Radarr", "Radarr", self.settings.url("radarr"), self.settings.key("radarr"), "v3", "/media/movies", "movies", 30987),
            ArrInstance("radarr-4k", "Radarr 4K", "Radarr", self.settings.url("radarr-4k"), self.settings.key("radarr-4k"), "v3", "/media/movies-4k", "movies-4k", 30988, True),
            ArrInstance("lidarr", "Lidarr", "Lidarr", self.settings.url("lidarr"), self.settings.key("lidarr"), "v1", "/media/music", "music", 30993),
        ]
    ...
    def configure_storage(self):
        roots = [
            ...
            Path("/media/series"),
            Path("/media/series-4k"),
            Path("/media/music"),
        ]
        for root in roots:
            root.mkdir(parents=True, exist_ok=True)
        probe = Path("/media/.umbrel-arr-write-test")
        probe.write_text("ok")
        probe.unlink()
        return "Local downloads and network media roots are writable"
